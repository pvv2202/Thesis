...
from Random.random_utils import broadcastable, get_dim_size, mul_broadcastable
...
class Instructions:
    ...
    # TODO: Doesn't support multiplying 1D tensors right now.
    @staticmethod
    def tensor_matmul(stack):
        ...
        if len(stack['tensor']) >= 2:
            ...
            if not mul_broadcastable(stack['tensor'][-1].size(), stack['tensor'][-2].size()):
                return
